...
def extract_youtube_id(text):
    ...
    if len(cleaned) == 11 and re.match(r'^[a-zA-Z0-9_-]{11}$', cleaned):
        return cleaned
    patterns = [
        r'(?:v|/v/|embed/|youtu\.be/|/watch\?v=|&v=|\?v=)([a-zA-Z0-9_-]{11})'
    ]
    ...
    try:
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            with urllib.request.urlopen(req_main, timeout=5) as resp_main:
                ...
                if not playable:
                    return False, title, False, 0, "This video cannot be played (copyright or age-restricted)"
                if not embeddable:
                    return True, title, False, duration_seconds, "The uploader has disabled playback outside of YouTube (Embedding disabled)"
                return True, title, True, duration_seconds, "OK"
    except Exception as e:
        return False, "Unknown Video", False, 0, f"Error verifying video: {e}"
...
def search_youtube(query):
    ...
    return None, None, False, 0, "No playable results found on the web"
...
def set_pc_volume(volume_percent):
    ...
    if not has_pycaw:
        print("[WARNING] pycaw not available. Volume cannot be set.")
        return
    try:
        import comtypes
        try:
        ...
        print(f"[VOLUME] Set master volume to {vol}%")
    except Exception as e:
...
class PCAgentHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        global current_song, music_queue, last_player_poll_time, is_playing, player_action
        ...
        if path == "/":
            ...
            is_player = query.get("role", [""])[0] == "player"
            if is_player:
            ...
            if cmd == "play":
                if current_song:
                    ...
                    player_action = "play" if is_playing else "pause"
                else:
                    press_key(VK_MEDIA_PLAY_PAUSE)
            elif cmd == "next":
                if current_song or music_queue:
                    close_direct_youtube()
                    if music_queue:
                        current_song = music_queue.pop(0)
                        is_playing = True
                ...
                try:
                    ...
                    print(f"[ERROR] Failed to lock workstation: {e}")
            elif cmd == "desktop":
                try:
                    ...
                    print(f"[ERROR] Failed to show desktop: {e}")
            elif cmd == "calc":
                import subprocess
                ...
                val = query.get("val", [""])[0]
                if val:
                    url = "https://www.google.com/search?q=" + urllib.parse.quote(val)
                    import webbrowser
                    webbrowser.open(url)
            elif cmd == "ytmusic":
...
def main():
    ...
    print("====================================================")
    print("      ComMSP System Starting | icexerspace          ")
    print("====================================================")
    ...
    print(f"✨ [DIRECT CONTROL] Access the control panel directly at: http://{local_ip}:8000")
